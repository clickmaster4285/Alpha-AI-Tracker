// Alpha AI Tracker — Browser Journey (Gecko engine)
// Generic WebExtensions background: prefers browser.* with chrome.* fallback.
// Engine tag in payloads is always "gecko" — never a browser brand name.
...
function connectNative() {
  ...
  try {
    ...
    setTimeout(() => {
      ...
      } catch (_) { /* reconnect via keepalive */ }
    }, 500);
...
function setupKeepalive() {
  if (api.alarms) {
    ...
    setInterval(pingOrReconnect, 25000);
  }
}

function pingOrReconnect() {
  if (!nativePort) {
    connectNative();
    return;
  }
...
console.log("[Alpha AI] Browser Journey extension loaded (gecko)");
