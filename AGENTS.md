...
> **Changelog:**
>
> - 2026-08-31: **Server: Time & Attendance Phase 2 APIs.** Migration 028 adds shift IANA timezones, `company_holidays`, and aggregate-capable session-event fields (`event_count`, `first_at`, `last_at`). New endpoints: device-authenticated `GET /api/v1/schedules/me`, public `GET /api/v1/server-time`, JWT-protected holiday CRUD, and paginated `GET /api/v1/attendance/today|range`. Attendance is computed server-side from events + heartbeat + schedule + holiday rules; overlapping idle and screen-lock intervals are unioned. Shift CRUD and the web Shift Management form expose timezone so local workday boundaries are explicit. The desktop skew probe now uses `/server-time`. Verified: `go test ./...`, `go vet ./...`, client `dotnet build`, and web TypeScript checking pass; live migration/API smoke testing requires a configured server `.env`.
