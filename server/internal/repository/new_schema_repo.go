	...
	for i := 0; i < len(entries); i += batchSize {
		...
		for _, e := range batch {
			...
			args = append(args,
				...
				e.CpuModel, e.CpuCores, e.RamTotalMB, e.StorageDevices, e.GpuModel, e.GpuVramMB, e.CollectedAt, time.Now(),
			)
			argIdx += 14
		}
	...
	for i := 0; i < len(entries); i += batchSize {
		...
		for _, e := range batch {
			...
			args = append(args,
				...
				e.EndedAt, e.MachineID, e.SessionID, e.Platform, time.Now(),
			)
			argIdx += 10
		}
		...
		query := fmt.Sprintf(`
			INSERT INTO app_sessions
				(id, employee_id, process_name, app_display_name, started_at,
				 ended_at, machine_id, session_id, platform, synced_at)
			VALUES %s
			ON CONFLICT (id) DO NOTHING
		`, strings.Join(valueStrings, ", "))
...
func (r *NewSchemaRepo) ListAppSessions(ctx context.Context, params AppSessionListParams) (*AppSessionListResult, error) {
	...
	query := fmt.Sprintf(`
		SELECT id, employee_id, process_name, app_display_name, started_at, ended_at,
		       machine_id, session_id, platform, synced_at, created_at
		FROM app_sessions %s
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&s.MachineID, &s.SessionID, &s.Platform, &s.SyncedAt, &s.CreatedAt,
		); err != nil {
