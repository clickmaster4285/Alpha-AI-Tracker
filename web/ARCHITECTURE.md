...
> **Last audited:** 2026-09-04 (per-app usage aggregate + chevron expand for /employee-journey/apps)
> **Changelog:**
> - 2026-09-04: **`/employee-journey/apps` chevron expand — per-app session list under each app row.** Each app row now has a chevron. Clicking it expands an inline table listing every individual `app_sessions` row for that `(appDisplayName, processName)` pair: Opened, Closed, Duration (3-state-aware: CLOSED → `endedAt`, STALE/OFFLINE → `lastSyncAt`, ACTIVE → `now`), Process (binary name like `chrome`), Title (the `contextLabel` — window title at open), Foreground, Background (the 2026-08-15 focus totals in seconds). The expand fires its own paginated server call (`GET /app-sessions/usage/sessions?appDisplayName=X&processName=Y&page=N&perPage=20`, registered BEFORE `/app-sessions/usage` so a future `:id` route can't swallow it). 20/page cap keeps the per-page payload small for users with 200+ chrome opens/week. The inner list uses `useInfiniteQuery` per the *Web Infinite-Scroll Rule* — the sentinel is a "Load more sessions" button (the inline area is short — IntersectionObserver adds no value here); a "Showing all N" footer appears when `hasNextPage` is false. The expand state is per-page `useState` (not URL) — the expand/collapse is a per-user UI view, not a shareable filter. The page does NOT fetch on initial load — the 100-row aggregate is still one HTTP call; the inner fetch is purely on chevron click. New `appSessionsApi.usageSessions({appDisplayName, processName, page, perPage, employeeId, dateFrom, dateTo})` in `lib/api.ts`; `AppUsageRow` is unchanged. Route size: `/employee-journey/apps` 2.89 kB → 4.34 kB. Cross-service contract: server `new_schema_repo.ListAppSessionsForApp` reuses the existing `AppSessionListResult` shape so the service is a one-liner; the new handler returns 400 when both keys are empty (a single key without its pair would be ambiguous). Verified: `npx tsc --noEmit` clean, `next build` passes. Manual test plan in `TESTING.md` (requires live pg + redis + server).
