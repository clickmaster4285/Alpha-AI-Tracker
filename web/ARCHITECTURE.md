...
> **Changelog:**
> - 2026-09-14: **Per-feature T&C consent history page.** New `/settings/terms-and-conditions` page with styled T&C content per feature (required/optional badges, accepted/revoked status, per-section headings). Uses `termsConsentApi.list()` from `lib/api.ts` and `useQuery` to fetch consent records per employee. Verified: `npx tsc --noEmit` clean, `next build` passes (`/settings/terms-and-conditions` 6.27 kB).
...
| `/settings/terms-and-conditions` | **Per-feature T&C content** — shows detailed T&C descriptions (What We Track, How Data Is Used, Data Retention, Your Rights) for each tracking feature, with required/optional and accepted/revoked status badges. Uses `termsConsentApi.list()` from `lib/api.ts`. | Server (`/terms-consent`) | ✅ |
