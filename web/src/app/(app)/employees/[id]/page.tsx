...
import {
  ...
  type DeviceHardwareDetail, type StorageDeviceDetail, type NetworkInfoDetail,
} from '@/lib/api';
...
function ActivityTab({ employeeId }: { employeeId: string }) {
  ...
  // Server-side pagination with infinite scroll: each page is fetched from
  // GET /app-sessions?employeeId=&page=&perPage= and appended as the user scrolls.
  const {
    data,
    isLoading,
    isError,
...
function FocusTime({ fg, bg }: { fg: number; bg: number }) {
  const total = fg + bg;
  if (total <= 0) {
